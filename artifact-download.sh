...
Help()
{
   ...
   echo "[version-output]     This is an optional output if you need a different version/directory name."
   echo
}
...
if [ -z "$2" ]
  then
    ...
    # This doesn't actually need the second positional arg but it's easier to just pass it
    Setarch $1 $2 arm64
    fi
  if [ -z "$2" ]
  then
